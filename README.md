...
- **Runs entirely locally** (Ollama) — no API key, no per-call cost, and no project
  data ever leaves the machine
- **Three-tier reliability**: live model call → short-lived cache → deterministic
  rule-based fallback if the model is unavailable for any reason. The page never errors
...
The insights panel is powered by a **language model running locally on your own
machine** via [Ollama](https://ollama.com). No project data is sent to any hosted
service, and there is no API key.

This is a deliberate design constraint, not a convenience. The tool is built to hold a
plant operator's live capital-project data — schedules, budgets, contractor names,
personnel. Posting that to a third-party API endpoint is not something an operator's IT
function would approve, so a design that depends on a hosted model isn't deployable in
the environment this tool is actually for. Running inference locally also means the
portal has no per-call cost and works fully offline, which the rest of the project (no
CDN, no web fonts, no charting library) was already built for.

The layering is unchanged and the feature remains entirely optional:
...
# 3. That's it. Ollama serves on 127.0.0.1:11434 and the portal finds it automatically.
```

On a machine with limited RAM, `ollama pull llama3.2:1b` is lighter and still works;
set `PEER_AI_MODEL=llama3.2:1b` to use it. If Ollama isn't installed or isn't running,
the connection is refused immediately (no hang) and the app runs on the fallback below.

### How insights work *without* AI

This is the important part for reliability: **the app never depends on the AI being
available.** If Ollama isn't installed or running, the model hasn't been pulled, the
call times out, or the response is unparseable, `ai_insights.py` falls back to a
**rule-based narrative generator** — plain Python functions (`_rule_based_project`, `_rule_based_portfolio`)
