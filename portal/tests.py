...
class AIFallbackTests(TestCase):
    ...
    def test_falls_back_when_local_model_is_switched_off(self):
        with self.settings(AI_ENABLED=False, AI_CACHE_SECONDS=0):
    ...
    def test_survives_ollama_not_running(self):
        with self.settings(AI_ENABLED=True, AI_CACHE_SECONDS=0), \
    ...
    def test_extracts_text_from_ollama_chat_response_shape(self):
        body = {"message": {"role": "assistant",
                            "content": '{"summary": "S", "risks": [], "actions": []}'},
                "done": True}
        self.assertEqual(
            ai_insights._extract_text(body),
            '{"summary": "S", "risks": [], "actions": []}',
        )

    def test_extracts_text_from_ollama_generate_response_shape(self):
        """AI_API_URL can be pointed at /api/generate instead of /api/chat."""
        body = {"response": '{"summary": "S", "risks": [], "actions": []}', "done": True}
        self.assertEqual(
            ai_insights._extract_text(body),
            '{"summary": "S", "risks": [], "actions": []}',
        )

    def test_handles_model_not_pulled(self):
        with self.settings(AI_ENABLED=True, AI_CACHE_SECONDS=0), \
             mock.patch("portal.ai_insights._call_model",
                        return_value=(None, "Local model 'llama3.2' not found.")):
            insight = ai_insights.project_insight(self.analysis)
        self.assertEqual(insight["source"], "rules")
        self.assert_usable(insight)
